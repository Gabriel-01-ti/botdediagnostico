...
// ================= CARREGAR BASE =================
fetch("base.json").then(res => res.json()).then(data => {
   baseDados = data;
...
function mostrarBotoesSintomas() {
   ...
   // Estrutura organizada com divs separadas
   let html = `<div class="sintomas-container">`;
   html += `<div class="sintomas-botoes">`;
   
   sintomasSet.forEach(s => {
      html += `<button class="btn-sintoma" onclick="toggleSintoma(this, '${s.replace(/'/g, "\\'")}')">${s}</button>`;
   });
   
   html += `</div>`; // fecha .sintomas-botoes
   html += `<button class="btn-diagnosticar-acao" onclick="finalizarSelecao()">🔍 Diagnosticar</button>`;
   html += `</div>`; // fecha .sintomas-container
...
btnEnviar.addEventListener("click", () => {
   ...
   if (etapa === 0) iniciarBot();
      ...
      if (["sim", "s", "quero", "claro"].includes(resposta)) {
         ...
         addMsg("Sem problema 😊<br>Digite o nome da cultura (Ex: Milho, Soja, Feijão).", "bot");
         etapa = 1;
      } else {
...
Como você prefere fazer o diagnóstico?<br><br>
  ...
  <div class="botoes-opcao"><button onclick="escolherModo('texto')">✍️ Descrever sintomas</button>
